      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between py-4 gap-4">
          ...
          <div className="flex items-center justify-center bg-zinc-900/90 p-1.5 rounded-xl border border-zinc-800 shadow-inner">
            <button onClick={() => onMonthChange(getAdjacentMonthKey(currentMonthKey, -1))} className="p-1.5 rounded-lg hover:bg-zinc-800 text-zinc-300 hover:text-white transition-colors" title="Mes anterior" id="prev-month-btn"><GradientIcon icon={ChevronLeft} className="w-5 h-5" strokeWidth={2.4} /></button>
            ...
            <button onClick={() => onMonthChange(getAdjacentMonthKey(currentMonthKey, 1))} className="p-1.5 rounded-lg hover:bg-zinc-800 text-zinc-300 hover:text-white transition-colors" title="Mes siguiente" id="next-month-btn"><GradientIcon icon={ChevronRight} className="w-5 h-5" strokeWidth={2.4} /></button>
            {!isCurrentMonth && <button onClick={() => onMonthChange(getCurrentMonthKey())} className="ml-2 text-xs px-2.5 py-1 rounded-md bg-orange-500/10 text-orange-400 border border-orange-500/30 hover:bg-orange-500/20 font-bold transition-colors" id="current-month-btn">Hoy</button>}
          </div>
        ...
        <div className="flex items-center space-x-1 border-t border-zinc-800/80 pt-2 pb-2 overflow-x-auto no-scrollbar">
          <button onClick={() => onTabChange('dashboard')} className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${activeTab === 'dashboard' ? 'bg-zinc-900 text-orange-400 border border-orange-500/40 shadow-sm' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900/60'}`} id="tab-dashboard"><GradientIcon icon={LayoutDashboard} className="w-4 h-4" strokeWidth={2.2} /><span>Resumen General</span></button>
          <button onClick={() => onTabChange('transactions')} className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${activeTab === 'transactions' ? 'bg-zinc-900 text-orange-400 border border-orange-500/40 shadow-sm' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900/60'}`} id="tab-transactions"><GradientIcon icon={ListOrdered} className="w-4 h-4" strokeWidth={2.2} /><span>Movimientos</span></button>
          <button onClick={() => onTabChange('budgets')} className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${activeTab === 'budgets' ? 'bg-zinc-900 text-orange-400 border border-orange-500/40 shadow-sm' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900/60'}`} id="tab-budgets"><GradientIcon icon={Target} className="w-4 h-4" strokeWidth={2.2} /><span>Presupuestos</span></button>
          <button onClick={() => onTabChange('analytics')} className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${activeTab === 'analytics' ? 'bg-zinc-900 text-orange-400 border border-orange-500/40 shadow-sm' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900/60'}`} id="tab-analytics"><GradientIcon icon={PieChart} className="w-4 h-4" strokeWidth={2.2} /><span>Estadísticas</span></button>
          <button onClick={() => onTabChange('categories')} className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${activeTab === 'categories' ? 'bg-zinc-900 text-orange-400 border border-orange-500/40 shadow-sm' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900/60'}`} id="tab-categories"><GradientIcon icon={Settings} className="w-4 h-4" strokeWidth={2.2} /><span>Categorías</span></button>
        </div>
      </div>
    </header>
  );
