...
export const SupabaseSyncModal: React.FC<Props> = ({ isOpen, onClose, transactions, categories, budgets, settings }) => {
  ...
  const [lastSyncError, setLastSyncError] = useState<LastSupabaseSyncError | null>(() => getLastSupabaseSyncError());
  ...
  useEffect(() => {
    ...
    const handleSyncStatus = () => setLastSyncError(getLastSupabaseSyncError());
  ...
  const handleMigration = async () => {
    ...
    try { const result = await migrateLocalDataToSupabase(userId, { transactions, categories, budgets, settings }); setMessage(`Migración verificada: ${result.transactions} movimientos, ${result.categories} categorías y ${result.budgets} presupuestos. Tus datos locales siguen intactos.`); setLastSyncError(getLastSupabaseSyncError()); }
    catch (e) { setError(e instanceof Error ? e.message : 'La migración no pudo completarse.'); setLastSyncError(getLastSupabaseSyncError()); }
    finally { setLoading(false); }
  };
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4"><div className="w-full max-w-lg bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden">
    <div className="flex items-center justify-between p-5 border-b border-zinc-800"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-orange-500/10 border border-orange-500/30 flex items-center justify-center"><Cloud className="w-5 h-5 text-orange-400" /></div><div><h2 className="font-extrabold text-white">Nube y respaldo</h2><p className="text-xs text-zinc-500">Supabase · diagnóstico seguro</p></div></div><button onClick={onClose} className="p-2 text-zinc-400 hover:text-white rounded-lg hover:bg-zinc-900"><X className="w-5 h-5" /></button></div>
    <div className="p-5 space-y-5"><div className="flex gap-3 rounded-xl bg-zinc-900/70 border border-zinc-800 p-4 text-sm text-zinc-300"><ShieldCheck className="w-5 h-5 shrink-0 text-emerald-400" /><p>Este diagnóstico no borra movimientos, no limpia localStorage, no modifica RLS y no reemplaza tus datos locales.</p></div>
      {lastSyncError && <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-4 space-y-2"><div className="flex items-center gap-2 text-red-300 font-bold text-sm"><AlertTriangle className="w-4 h-4" /> Último error real de sincronización</div><p className="text-xs text-zinc-400">Operación: <span className="text-zinc-200">{lastSyncError.label}</span></p><p className="text-sm text-red-200 break-words">{lastSyncError.message}</p><p className="text-[11px] text-zinc-500">Registrado: {formatDiagnosticDate(lastSyncError.timestamp)}</p></div>}
