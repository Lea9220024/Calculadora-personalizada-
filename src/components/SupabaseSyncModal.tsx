...
export const SupabaseSyncModal: React.FC<Props> = ({ isOpen, onClose, transactions, categories, budgets, settings }) => {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4"><div className="w-full max-w-lg bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden">
    ...
    <div className="p-5 space-y-5"><div className="flex gap-3 rounded-xl bg-zinc-900/70 border border-zinc-800 p-4 text-sm text-zinc-300"><ShieldCheck className="w-5 h-5 shrink-0 text-emerald-400" /><p>La migración es manual. No borra localStorage, no reemplaza todavía la fuente actual y puede repetirse sin crear duplicados.</p></div>
      {!userId ? <div className="space-y-3"><h3 className="font-bold text-white">Iniciar sesión</h3><input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="Tu email" className="w-full rounded-xl bg-zinc-900 border border-zinc-800 px-4 py-3 text-sm text-white outline-none focus:border-orange-500" /><input value={password} onChange={e => setPassword(e.target.value)} type="password" placeholder="Tu contraseña" className="w-full rounded-xl bg-zinc-900 border border-zinc-800 px-4 py-3 text-sm text-white outline-none focus:border-orange-500" /><button disabled={loading || !email || !password} onClick={handleAuth} className="w-full flex items-center justify-center gap-2 rounded-xl bg-orange-500 hover:bg-orange-400 disabled:opacity-40 px-4 py-3 text-sm font-extrabold text-black"><LogIn className="w-4 h-4" /> Iniciar sesión</button><p className="text-xs text-zinc-500">La cuenta debe existir previamente en Supabase Auth. No se crea ninguna cuenta automáticamente.</p></div>
