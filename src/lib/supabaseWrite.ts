...
const saveLastSupabaseSyncError = (label: string, message: string) => {
  ...
  try {
    ...
    // El diagnóstico no debe interrumpir nunca la operación principal.
  }
};
...
export async function syncBudgetToSupabase(budget: MonthlyBudget, reportStatus = true): Promise<void> {
  await withRetry(async () => {
    const userId = await getAuthenticatedUserId();
    if (!supabase) throw new Error('Supabase no está configurado.');
