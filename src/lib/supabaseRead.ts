...
function mergeTransactions(cloudTransactions: Transaction[]): Transaction[] {
  ...
  for (const transaction of localOnlyTransactions) enqueuePendingSupabaseSync({ type: 'upsert_transaction', payload: transaction });
...
export async function readUserDataFromSupabase(userId: string): Promise<SupabaseDataPayload> {
  ...
  const cloudTransactions = (transactionsResult.data ?? []).map((row) => ({
    id: row.id, title: row.title, amount: Number(row.amount), type: row.type, categoryId: row.category_id,
    date: row.date, paymentMethod: row.payment_method, notes: row.notes ?? undefined, isRecurring: row.is_recurring ?? false,
    cardId: row.card_id ?? undefined, installmentPlanId: row.installment_plan_id ?? undefined,
    installmentNumber: row.installment_number ?? undefined, installmentTotal: row.installment_total ?? undefined,
    installmentTotalAmount: row.installment_total_amount == null ? undefined : Number(row.installment_total_amount), createdAt: row.created_at,
  }));
