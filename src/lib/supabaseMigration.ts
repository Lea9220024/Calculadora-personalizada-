...
export async function migrateLocalDataToSupabase(
  ...
  for (const budget of payload.budgets) {
    const { data: existingBudget, error: findBudgetError } = await supabase
    ...
    if (findBudgetError) throw new Error(`Presupuesto ${budget.monthKey}: ${findBudgetError.message}`);
    ...
    const { error: budgetError } = existingBudget
    ...
    if (budgetError) throw new Error(`Presupuesto ${budget.monthKey}: ${budgetError.message}`);
  }
  ...
  const { error: settingsError } = await supabase
    ...
    .upsert(
      {
        user_id: userId,
        currency_symbol: payload.settings.currencySymbol,
        currency_code: payload.settings.currencyCode,
        theme: payload.settings.theme,
        start_day_of_month: payload.settings.startDayOfMonth,
      },
      { onConflict: 'user_id' }
    );
