...
export async function migrateLocalDataToSupabase(userId: string, payload: MigrationPayload): Promise<MigrationResult> {
  ...
  const { error: categoryError } = await supabase.from('calculator_categories').upsert(payload.categories.map((category) => ({
    id: category.id, user_id: userId, name: category.name, icon: category.icon, color: category.color, text_color: category.textColor, type: category.type,
  })), { onConflict: 'id' });
  if (categoryError) throw new Error(`Categorías: ${categoryError.message}`);

  const { error: transactionError } = await supabase.from('calculator_transactions').upsert(payload.transactions.map((transaction) => ({
    id: transaction.id, user_id: userId, title: transaction.title, amount: transaction.amount, type: transaction.type, category_id: transaction.categoryId,
    date: transaction.date, payment_method: transaction.paymentMethod, notes: transaction.notes ?? null, is_recurring: transaction.isRecurring ?? false, created_at: transaction.createdAt,
  })), { onConflict: 'id' });
  ...
  for (const budget of payload.budgets) {
    ...
    const { error } = existingBudget
      ? await supabase.from('calculator_monthly_budgets').update(budgetData).eq('id', existingBudget.id)
  ...
  const { error: settingsError } = await supabase.from('calculator_settings').upsert({
    user_id: userId, currency_symbol: payload.settings.currencySymbol, currency_code: payload.settings.currencyCode,
    theme: payload.settings.theme, start_day_of_month: payload.settings.startDayOfMonth,
  }, { onConflict: 'user_id' });
