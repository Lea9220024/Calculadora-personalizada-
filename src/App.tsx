...
export default function App() {
  ...
  useEffect(() => {
    ...
    const startRealtime = (userId: string) => {
      ...
      realtimeChannel = subscribeToCalculatorRealtime(userId, () => { void loadCloudData(userId); });
    };
    const loadCurrentSession = async () => {
      const { data: { session } } = await supabase.auth.getSession();
      if (!cancelled && session?.user) { await loadCloudData(session.user.id); startRealtime(session.user.id); }
    };
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === 'SIGNED_OUT' && realtimeChannel) {
        void unsubscribeFromCalculatorRealtime(realtimeChannel); realtimeChannel = null;
      }
    });
    return () => {
      cancelled = true;
      authListener.subscription.unsubscribe();
      if (realtimeChannel) void unsubscribeFromCalculatorRealtime(realtimeChannel);
    };
  }, []);
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 font-sans flex flex-col selection:bg-orange-500 selection:text-black">
      <GlobalGradientDefs />
      <Header currentMonthKey={currentMonthKey} onMonthChange={setCurrentMonthKey} activeTab={activeTab} onTabChange={setActiveTab}
        onOpenNewTransaction={() => { setEditingTransaction(null); setIsFormModalOpen(true); }} onOpenExportImport={() => setIsExportImportOpen(true)}
        onOpenSupabaseSync={() => setIsSupabaseSyncOpen(true)} currencySymbol={settings.currencySymbol} onCurrencyChange={handleCurrencyChange} />
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {activeTab === 'dashboard' && <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-7 space-y-6"><TransactionList transactions={monthTransactions} categories={categories} currencySymbol={settings.currencySymbol}
              onEdit={tx => { setEditingTransaction(tx); setIsFormModalOpen(true); }} onDuplicate={handleDuplicateTransaction} onDelete={handleDeleteTransaction}
              onAddNew={() => { setEditingTransaction(null); setIsFormModalOpen(true); }} /></div>
            <div className="lg:col-span-5 space-y-6"><BudgetOverview currentMonthKey={currentMonthKey} monthlyBudget={currentBudget} transactions={monthTransactions} categories={categories} currencySymbol={settings.currencySymbol} onUpdateBudget={handleUpdateBudget} /></div>
          </div>
          <FinancialIntelligence currentMonthKey={currentMonthKey} budgetTarget={currentBudget.totalTarget} transactions={monthTransactions} currencySymbol={settings.currencySymbol} />
          <FinancialGoals transactions={transactions} currencySymbol={settings.currencySymbol} />
          <AnalyticsCharts transactions={monthTransactions} categories={categories} currencySymbol={settings.currencySymbol} />
          <HistoricalAnalysis transactions={transactions} currencySymbol={settings.currencySymbol} />
        </div>}
        {activeTab === 'transactions' && <TransactionList transactions={monthTransactions} categories={categories} currencySymbol={settings.currencySymbol}
          onEdit={tx => { setEditingTransaction(tx); setIsFormModalOpen(true); }} onDuplicate={handleDuplicateTransaction} onDelete={handleDeleteTransaction}
          onAddNew={() => { setEditingTransaction(null); setIsFormModalOpen(true); }} />}
        {activeTab === 'budgets' && <><FinancialIntelligence currentMonthKey={currentMonthKey} budgetTarget={currentBudget.totalTarget} transactions={monthTransactions} currencySymbol={settings.currencySymbol} />
          <div className="mt-6"><BudgetOverview currentMonthKey={currentMonthKey} monthlyBudget={currentBudget} transactions={monthTransactions} categories={categories} currencySymbol={settings.currencySymbol} onUpdateBudget={handleUpdateBudget} /></div></>}
        {activeTab === 'analytics' && <><AnalyticsCharts transactions={monthTransactions} categories={categories} currencySymbol={settings.currencySymbol} />
          <HistoricalAnalysis transactions={transactions} currencySymbol={settings.currencySymbol} /><div className="mt-6"><FinancialGoals transactions={transactions} currencySymbol={settings.currencySymbol} /></div></>}
